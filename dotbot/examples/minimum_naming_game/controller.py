...
class Controller:
    def __init__(self, address: str, path: str):
        ...
        # Inventory of known words (set is ideal for uniqueness)
        # Using a set replaces the need for 'inventory_size' as a separate variable
        self.inventory = set()
    ...
    def _callback_selectAndBroadcast(self, data: any):
            ...
            if not self.inventory:
                ...
                # Note: random.choice requires a sequence (list/tuple), so we convert the set
    ...
    def _callback_updateInventory(self, data: any):
            ...
            # (Equivalent to the C for-loop and word_known flag)
    ...
    def _check__selectAndBroadcast(self, data: any) -> bool:
        ...
        if self.new_word_received:
            # Reset the flag (mirroring 'new_word_received = false')
            self.new_word_received = False
            return True
    ...
    def color_code(self):
        ...
        # In Python, we check the length of the set
